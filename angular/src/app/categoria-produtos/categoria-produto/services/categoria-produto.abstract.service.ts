...
} from '../../../proxy/categoria-produtos/models';
import { CategoriaProdutoService } from '../../../proxy/categoria-produtos/categoria-produto.service';

export abstract class AbstractCategoriaProdutoViewService {
  protected readonly proxyService = inject(CategoriaProdutoService);
  protected readonly confirmationService = inject(ConfirmationService);
  protected readonly list = inject(ListService);

  public readonly getWithNavigationProperties = this.proxyService.getWithNavigationProperties;

  data: PagedResultDto<CategoriaProdutoWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<CategoriaProdutoWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetCategoriaProdutosInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ categoriaProduto: { id } }) => id);
  ...
  delete(record: CategoriaProdutoWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.categoriaProduto.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<CategoriaProdutoWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetCategoriaProdutosInput;
    this.list.get();
  }
}
